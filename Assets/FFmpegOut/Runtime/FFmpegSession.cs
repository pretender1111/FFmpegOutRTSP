...
namespace FFmpegOut
{
    public sealed class FFmpegSession : System.IDisposable
    {
        ...
        public static FFmpegSession Create(
            string name,
            int width, int height, float frameRate, string url,
            FFmpegPreset preset
        )
        {
            name += System.DateTime.Now.ToString(" yyyy MMdd HHmmss");
            var path = name.Replace(" ", "_") + preset.GetSuffix();
            return CreateWithOutputPath(path, width, height, frameRate, url, preset);
        }
        ...
        //将给定的纹理进行预处理，并将预处理后的渲染纹理添加到异步GPU读取请求队列中
